 ...
 *
 * This repository is a service named 'testing_example.repository'. You can see
 * how the service is defined in testing_example/testing_example.services.yml.
 *
...
class DbtngExampleRepository {
  ...
  public function insert(array $entry) {
    try {
      $return_value = $this->connection->insert('testing_example')
        ->fields($entry)
        ->execute();
    }
    catch (\Exception $e) {
  ...
  public function update(array $entry) {
    try {
      ...
      $count = $this->connection->update('testing_example')
        ->fields($entry)
  ...
  public function delete(array $entry) {
    $this->connection->delete('testing_example')
      ->condition('pid', $entry['pid'])
      ->execute();
  }
   ...
   *
   * @code
   *   // SELECT * FROM {testing_example} WHERE uid = 0 AND name = 'John'
   *   \Drupal::database()->query(
   *     "SELECT * FROM {testing_example} WHERE uid = :uid and name = :name",
   *     [':uid' => 0, ':name' => 'John']
   *   )->execute();
   * @endcode
   *
   ...
   * @code
   *   // SELECT * FROM {testing_example} WHERE uid = 0 AND name = 'John'
   *   \Drupal::database()->select('testing_example')
   *     ->fields('testing_example')
   *     ->condition('uid', 0)
   *     ->condition('name', 'John')
   *     ->execute();
   * @endcode
   ...
   * altered:
   * @code
   *   // SELECT * FROM {testing_example} WHERE age > 18
   *   \Drupal::database()->select('testing_example')
   *     ->fields('testing_example')
   *     ->condition('age', 18, '>')
   *     ->execute();
  ...
  public function load(array $entry = []) {
    // Read all the fields from the testing_example table.
    $select = $this->connection
      ->select('testing_example')
  ...
  /**
   * Load testing_example records joined with user records.
   *
  ...
  public function advancedLoad() {
    // Get a select query for our testing_example table. We supply an alias of e
    // (for 'example').
    $select = $this->connection->select('testing_example', 'e');
